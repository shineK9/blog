...
description: 系统化聊聊前端工程，梳理一下相关的知识点，解释什么是前端工程化。顺便谈谈现在主流的工程化技术。
tags: '工程化,编译'
createOn: '2023-3-04'
---

## 前言
这篇文章的目的，是为了科普软件工程在前端领域的应用，我想尽量告诉大家每个技术是因为什么原因产生的，
以及它解决了什么问题。首先我们先科普一下前端工程是什么，让大家有一个共识。之后依靠这个共识，
引出一些前端工程面临的问题，然后尝试去解决它们。
...
类型系统的健壮性，是判断一门编程语言是否适合开发工业级应用的重要条件。

类型系统对项目提供：
- 更好的编辑器提示，让你能更方便、准确的输入代码。
- 编译器能帮你找出更多的代码错误，不需要等到运行时才发现，提高开发效率。
- 帮你描述抽象概念，对数据进行建模，起到了一定的文档作用。

这些特性，对于代码工程来说都是很有帮助的。可以参考 Java/C++/C# 这些比较流行的工业语言，
他们都有一个非常强大的类型系统。好的类型系统设计，甚至能帮助编程者更好的思考问题，比如
基于 class 的类型系统诞生了很多的设计模式，设计原则。人们称他们为面向对象(OOP)编程，基于 OOP
的设计模式也让程序员对某些业务场景应该如何设计有一个大概的共识，基本上精通 OOP 编程的
程序员，都用在较短时间内理解对方的代码架构，然后上手开发，这也从侧面大大提高了项目的可维护性。

让我们说回前端来，`JavaScript` 的原型设计仅仅花了 7 天，它的设计目标是一个动态、弱类型的脚本辅助类语言，
除了有一个简陋版类似 Java 的继承特性， 以及几种基本数据类型外，基本上就和类型系统沾不上边了。
可以说是 `JavaScript` 的定位变了，也可以说是为了弥补它在类型系统上的缺陷，微软推出了 `TypeScript`
编程语言，它不是另起炉灶，而是一个 `JavaScript` 的超集。什么意思呢？就是说它的语法是建立在
`JavaScript` 之上，附加了一个类型系统。

这是一段 `JavaScript` 的代码，你可以看到变量的类型取决于赋值，你可一个给一个变量赋不同数据类型的值。
...
再来看上面那段代码的 TypeScript 版本，基本上和 JavaScript 写法一致，但是它可以指定变量的类型，
并且类型确定后就不能改变了，如果你要这样做，编译器就会给你提示一个类型错误。
...
从上面的代码段可以看出来，如果你会 JavaScript，TypeScript 其实区别不大，你可以很快的上手。

除了这些基本的类型定义外，你还可以通过高级的类型去定义一些抽象的概念。这里我们可以举一个例子，
如何使用 TypeScript 定义我们熟悉的扑克牌:
...
`type` 关键词表示定义一个类型，这里我们定义了扑克的花色 Suit,
以及所有的卡牌类型：爵位牌 JQK，数字牌 A-10，大小王，'|' 符号表示“或者”的含义，你可以通过它
去给一个类型定义多个不同的成员，它们都会和该类型正确匹配，interface 定义单张扑克的结构，它包括
花色和卡牌类型。`TypeScript` 用短短几行代码就对能对整幅扑克牌的概念进行了定义，同时，这些类型
可以指导我们进行编程，在你错误的使用它们时及时给出提示。

例如我们想创建一张红桃 A，我们可以这样写，编译器会去检测变量的值是否和类型相对应，例如 suit 字段的值
会去匹配 `Suit` 类型中的成员，card 字段会去匹配 `PorkerCard` 类型中的成员，保证你的系统中不存在预期
外的值。
...
如果你不小心写错了，编译器就会给你提示，给你指出问题。

```ts
const heartAce: Porker = {
  suit: 'Hearta', // Type '"Hearta"' is not assignable to type 'Suit'. Did you mean '"Heart"'? (ts 2820)
	card: 'Ace'
}
```

总之，给前端项目引入一个类型系统是非常重要的，它能让我们的代码更加容易理解、更加健壮。

## 建立代码规范

从小，我们就开始练习写字，老师会教我们什么是笔画？什么是字体结构？书写时要注意顺序。然后，
我们就开始根据书本上的标准字体开始练习。期初大家可能都写的歪歪扭扭，由于都是第一次，
没有人会对写字风格有什么独到的见解。

![小学生字迹](/小学生字迹.jpg){width=400}

但是初中之后，你就会发现，班上同学之间的字迹差别越来越大了，原来努力写的工整的特别工整，
写的潦草的压根看不清写的啥。我记得我初中的时候，有些同学还会用一把尺子瞄着行线写字，
向下溢出的笔画会被挤成横线，这样写出来的字带着一种独特的个人色彩。

![字迹差异](/字迹差异.png)

那么问题来了，同样是对着一样的课本，用的一样的写法，为啥大家写的字会有越来越大的差别呢？
我觉得可能人性本就如此，每个人在产生个人主观意识后，连带的就会产生一个东西——创造力，
人喜欢做有创造力的事情，所有艺术作品都是人发挥创造力后诞生的产物，
它会让创造者和其他人心情愉悦。对于普通人来说，可能并不能创造出伟大的艺术作品，
但是也会倾向于在自己身边一些熟悉的事情上发挥自己的创造性。
写字就是这样的一件事情，因为没有规则的约束，每个人都会在其中加入一些自己的看法，
这就是造成了大家字迹越来越风格化的主要原因。

在代码编写这件事情上，人们发现也会存在这一现象，在做同样的一件事情时，
每一个程序员编写的代码可能都会有自己的一点个人风格。举个例子：
在 JavaScript 创建一个对象有下面好几种不同的写法，虽然作用都是相同的，但是实际上会在细节上有些区别。
...
对于软件工程来说，允许程序员的这种个人风格在项目中滥用，是会产生一些隐式问题的。上面也说了，
虽然带有风格的代码都能做成同一件事情，不过细节上是会有区别的，
如果一个项目中有存在很多这样的行为，积少成多，项目后期可能会产生一些怪异的 Bug，
并且很难被发现。

除了这些个人风格对项目的影响之外，还会存在一种问题。对于一个多人协作完成的软件项目，
往往只有少数几个人负责系统的架构、设计，其他人负责实现的开发者只了解系统某个局部的设计。
有时候会有一些宏观角度上优化系统的思想和方案，负责实现的开发者是不知道的，
需要设计人员将这些知识告诉负责实现的开发者。

总之，代码规范对于前端工程来说是很重要的。事实上，很多有技术影响力的互联网大厂也都有自己的代码规范。
下面列举一些知名的前端代码规范:

- [Airbnb Guide](https://github.com/airbnb/javascript): 目前前端领域最全面的开源代码规范
- [StandardJS](https://github.com/standard/standard): 提供一个标准化的 JavaScript 样式风格，顺便还提供了一系列的工具链用于落地。
